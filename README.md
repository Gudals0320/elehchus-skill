...
- 한 번 시작한 Codex 작업에서는 Elenchus 상태를 유지하며, 구현은 새 작업으로 분리한다.
...
새 Codex 작업에서 명시적으로 호출한다.

```text
$elenchus를 사용해 이 아이디어를 실행 전에 구체화해 줘.
```
...
Elenchus는 아이디어와 실행 계획을 확정하는 데서 끝난다. 제품 코드 구현, 개발 진행 추적과 계획 완료 뒤 같은 작업에서의 자동 개발 전환은 수행하지 않는다.
