...
REQUESTED = ROOT.parent / 'R001' / 'requested-samples' / '20260908T082652425301Z-3c0cfd0e9f00.json'
REQUESTED_REPEAT = ROOT.parent / 'R001' / 'requested-samples' / '20260908T082756854001Z-4fdfd2e8e75c.json'
REQUESTED_IMPERIAL = ROOT.parent / 'R001' / 'requested-samples' / '20260908T082801908167Z-905478802dbd.json'
...
class WeatherTests(unittest.TestCase):
    ...
    def test_requested_seoul_dates_two_variables_no_double_offset(self):
        record = weather.ingest(REQUESTED, self.db)
        self.assertEqual(record['rows'], 48)
        self.assertEqual(record['missing_rows'], 0)
        result = self.query(record['dataset_id'], '2026-09-01T00:00+09:00',
                            '2026-09-03T00:00+09:00', 'Asia/Seoul')
        self.assertEqual(len(result['rows']), 48)
        first, last = result['rows'][0], result['rows'][-1]
        self.assertEqual(first['epoch_utc'], 1788188400)  # 2026-08-31 15:00Z
        self.assertEqual(first['time_local'], '2026-09-01T00:00:00+09:00')
        self.assertEqual(last['time_local'], '2026-09-02T23:00:00+09:00')
        self.assertEqual(first['temperature_c'], 23.3)
        self.assertEqual(first['precipitation_mm'], 10.5)
        self.assertIsNone(first['wind_speed_ms'])
        self.assertEqual(first['quality'], 'not_requested:wind_speed_10m')
        self.assertTrue(result['summary']['precipitation_labels_complete'])

    def test_actual_repeat_and_same_file_replay_have_explicit_snapshot_policy(self):
        first = weather.ingest(REQUESTED, self.db)
        repeat = weather.ingest(REQUESTED_REPEAT, self.db)
        replay = weather.ingest(REQUESTED_REPEAT, self.db)
        self.assertNotEqual(first['dataset_id'], repeat['dataset_id'])
        self.assertFalse(replay['inserted'])
        self.assertEqual(weather.normalize(weather.read_json(REQUESTED.read_bytes())),
                         weather.normalize(weather.read_json(REQUESTED_REPEAT.read_bytes())))
        with closing(sqlite3.connect(self.db)) as connection:
            self.assertEqual(connection.execute('SELECT count(*) FROM datasets').fetchone()[0], 2)
            self.assertEqual(connection.execute('SELECT count(*) FROM hourly').fetchone()[0], 96)

    def test_requested_dates_imperial_units_match_with_rounding(self):
        si = weather.normalize(weather.read_json(REQUESTED.read_bytes()))
        imperial = weather.normalize(weather.read_json(REQUESTED_IMPERIAL.read_bytes()))
        self.assertEqual([r[0] for r in si], [r[0] for r in imperial])
        for left, right in zip(si, imperial):
            self.assertAlmostEqual(left[1], right[1], delta=.08)
            self.assertAlmostEqual(left[2], right[2], delta=.02)

    def test_requested_variable_omission_is_not_silently_accepted(self):
        del self.payload['hourly']['wind_speed_10m']
        del self.payload['hourly_units']['wind_speed_10m']
        with self.assertRaisesRegex(ValueError, 'variable sets differ'):
            self.ingest(self.payload)  # Metadata still requests wind: invalid response.

    def test_hourly_contract_objects_are_required(self):
        self.payload['hourly'] = []
