...
def request_url(latitude, longitude, start, end, imperial=False, request_timezone='UTC', include_wind=False):
    ...
    return 'https://archive-api.open-meteo.com/v1/archive?' + urlencode({
        ...
        'hourly': ','.join(FIELDS if include_wind else REQUIRED_FIELDS),
        'timezone': request_timezone, 'timeformat': 'unixtime',
...
def normalize(payload):
    ...
    if not isinstance(units, dict) or not isinstance(hourly, dict):
        raise ValueError('Expected hourly and hourly_units objects')
    ...
    active_fields = tuple(field for field in FIELDS if field in hourly or field in units)
    if not all(field in active_fields for field in REQUIRED_FIELDS):
        raise ValueError('Temperature and precipitation are required')
    absent_fields = [field for field in FIELDS if field not in active_fields]
    ...
    for index, epoch in enumerate(times):
        ...
        flags = []
        if nulls:
            flags.append('null:' + ','.join(nulls))
        if absent_fields:
            flags.append('not_requested:' + ','.join(absent_fields))
        rows.append((epoch, *values, ';'.join(flags) if flags else 'ok'))
        previous = epoch
    return rows
...
def ingest(raw_file, database):
    ...
    requested_fields = {field for part in query.get('hourly', []) for field in part.split(',')}
    if not set(REQUIRED_FIELDS) <= requested_fields <= set(FIELDS):
        raise ValueError('Unsupported requested variable set')
    if requested_fields != (set(payload['hourly']) - {'time'}):
        raise ValueError('Requested and returned variable sets differ')
    ...
    with closing(sqlite3.connect(database)) as connection:
        ...
        return {'dataset_id': identity, 'rows': len(rows), 'inserted': not bool(exists),
                'missing_rows': sum(row[-1] == 'missing_timestamp' or row[-1].startswith('null:') for row in rows)}
...
def main():
    ...
    download.add_argument('--start', default='2026-09-01')
    download.add_argument('--end', default='2026-09-02')
    download.add_argument('--timezone', default='Asia/Seoul', help='Calendar-day timezone used by the provider')
    download.add_argument('--include-wind', action='store_true', help='Optional third-variable comparison')
    ...
    try:
        if args.command == 'fetch':
            result = {'raw_file': str(fetch(request_url(args.latitude, args.longitude, args.start, args.end,
                                                       args.imperial, args.timezone, args.include_wind), args.out))}
