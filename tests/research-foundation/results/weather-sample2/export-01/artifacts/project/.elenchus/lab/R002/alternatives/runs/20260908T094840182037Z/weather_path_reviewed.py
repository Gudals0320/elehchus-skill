...
CREATE TABLE IF NOT EXISTS imports (
 ...
 raw_path TEXT NOT NULL, metadata_json TEXT NOT NULL, UNIQUE(sha256, url)
);
...
def ingest(raw_path, metadata_path, database):
    ...
    with closing(sqlite3.connect(database)) as connection:
        ...
        with connection:
            ...
            import_id = connection.execute('SELECT id FROM imports WHERE sha256=? AND url=?', (metadata['sha256'], metadata['url'])).fetchone()[0]
