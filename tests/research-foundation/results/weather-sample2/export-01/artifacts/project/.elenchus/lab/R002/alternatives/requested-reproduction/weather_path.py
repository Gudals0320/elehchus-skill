...
PRIMARY_VARIABLES = ('temperature_2m', 'precipitation')
...
def request_url(lat, lon, start, end, imperial=False, variables=VARIABLES, request_timezone='GMT'):
    ...
    if not variables or len(set(variables)) != len(variables) or not set(variables).issubset(VARIABLES):
        raise ContractError('Choose unique supported hourly variables')
    if request_timezone not in ('UTC', 'GMT', 'Asia/Seoul'):
        raise ContractError('Date requests currently support UTC/GMT or Asia/Seoul; display can use any installed IANA zone')
    get_timezone(request_timezone)
    params = dict(latitude=lat, longitude=lon, start_date=start, end_date=end,
                  hourly=','.join(variables), models='era5', timezone=request_timezone, timeformat='unixtime',
...
def contract(raw, metadata):
    ...
    def one(key):
        ...
        return values[0]
    if one('models') != 'era5' or one('timeformat') != 'unixtime':
        raise ContractError('Adapter requires explicit ERA5 and unixtime request')
    variables = tuple(one('hourly').split(','))
    requested_timezone = one('timezone')
    latitude, longitude = float(one('latitude')), float(one('longitude'))
    request_url(latitude, longitude, one('start_date'), one('end_date'), variables=variables, request_timezone=requested_timezone)
    source_tz = get_timezone(requested_timezone)
    start = int(datetime.fromisoformat(one('start_date') + 'T00:00:00').replace(tzinfo=source_tz).timestamp())
    stop = int(datetime.fromisoformat((date.fromisoformat(one('end_date')) + timedelta(days=1)).isoformat() + 'T00:00:00').replace(tzinfo=source_tz).timestamp())
    payload = json.loads(raw)
    if not isinstance(payload, dict) or payload.get('error'):
        raise ContractError('API error/unsupported response')
    expected_offset = int(datetime.fromtimestamp(start, source_tz).utcoffset().total_seconds())
    accepted_zones = ('UTC', 'GMT') if requested_timezone in ('UTC', 'GMT') else (requested_timezone,)
    if payload.get('timezone') not in accepted_zones or payload.get('utc_offset_seconds') != expected_offset or payload.get('hourly_units', {}).get('time') != 'unixtime':
...
def main():
    ...
    fetch_parser.add_argument('--lat', type=float, default=37.5665)
    fetch_parser.add_argument('--lon', type=float, default=126.9780)
    ...
    fetch_parser.add_argument('--variables', default=','.join(PRIMARY_VARIABLES))
    fetch_parser.add_argument('--request-timezone', default='Asia/Seoul', choices=('UTC', 'GMT', 'Asia/Seoul'))
    ...
    try:
        if args.command == 'fetch':
            raw, meta = fetch(request_url(args.lat, args.lon, args.start, args.end, args.imperial,
                                         tuple(args.variables.split(',')), args.request_timezone), args.out)
