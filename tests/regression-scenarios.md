...
- 기대 상태: 단일 사실의 간소화 조건으로 오인하지 않고 주장·계획·원문·관찰·반대 근거·적용 판단·공백을 추적한다. 한 비교·근거 표에 이 정보가 있으면 네 개의 별도 표를 다시 만들지 않는다.

### 비교 결과와 중복 기록

- 사전 상태: 여러 후보의 원문·관찰 수준·적용 판단·공백이 같은 R### 비교표에 기록되어 있고 Idea에 확정 제약이 있음
- 입력: Research 결과 정리
- 기대 상태: 기존 행·Idea 위치를 참조해 필요한 결론을 사용자에게 제시한다. 같은 제약과 근거를 여러 표·R###의 긴 문단으로 복사하느라 결과 전달을 지연하지 않는다. 추적할 정보나 다섯 완료 조건을 생략하는 것으로 분량을 줄이지 않는다.
